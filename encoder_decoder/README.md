...
$\widehat{\mathrm{SpO}}_2 = \mathrm{SpO}_{2,\mathrm{linear}} + \Delta$

where

$\Delta = \mathrm{max\_residual}\cdot\tanh\left(\mathrm{correction\_head}\left(\mathrm{ppg\_feature},\mathrm{r\_feature}\right)\right)$

The final layer of the correction head is initialized to zero. Therefore, at initialization,

$\widehat{SpO_2} = SpO_{2,\mathrm{linear}}$
...
$\mathcal{L}_{\mathrm{pretrain}} = \mathrm{MSE}\left([\widehat{\mathrm{RED}},\widehat{\mathrm{IR}}],[\mathrm{RED},\mathrm{IR}]\right)$
